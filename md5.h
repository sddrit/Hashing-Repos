 ...
 *
 * Compile with: gcc -o md5uwe md5uwe.c
 * Run:  ./md5uwe
...
// These vars will contain the hash. They are accessed by all the function as global variables
uint32_t h0, h1, h2, h3;
// the code will be h0[0], h0[1], h0[2], h0[3], h1[0],........h3[3]
// IN THIS ORDER so every number should be reversed when printing (low byte first to the left)
...
void calculateMd5(const char *input, uint8_t *result) {
    size_t len = strlen(input);
    md5((uint8_t *)input, len);

    uint8_t *p = result;

    // Store each byte of the hash in the result buffer
    for (int i = 0; i <= 3; ++i) {
        *(p++) = ((uint8_t *)&h0)[i];
    }
